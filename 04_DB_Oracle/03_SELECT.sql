...
2.데이터를 테이블 행태(행과 열)로 저장하고, 여러 테이블 간 관계를 설정하고, 데이터의 관계를 표현 및 분석
...
(Create - 추가, Read - 조회, Upadte - 수정, Delete - 삭제) -CRUD
-SELECT는 데이터 조회를 의미하여 일부에서는 DQL 이라고 한다(Date Query Language)로 분류하기도 함
...
이때 컬럼명에 별칭을 부여하면 깔끔하게 볼 수 있다.
컬럼 AS 별칭 / 컬럼 AS "별칭"/ 컬럼 별칭/ 컬럼"별칭"

*/
--USER_INFO에서 나이(AGE)에 10년후를 계산
SELECT NAME 이름,
AGE 나이,
AGE+10 AS"10년 후" 
FROM USER_INFO;
...
/*
리터럴(문자열)
-SELECT 절에 리터럴 사용하면 테이블에 존재하는 데이터처럼 조회 가능
...
--USER_INFO에서 이름(NAME)의 MBTI는XXXX입니다
SELECT NAME||'의 MBTI는'||MBTI||'입니다' MBTI FROM USER_INFO;

/*
DISTINCT
-컬럼에 중복된 값들을 한번씩만 표시하고자 할 때
-주의사항: DISTINCT는 SELECT절에서 한 번만 기술 기능
*/
...
--나이가25 이하면서 경기도에 사는 사람이거나 MBTI가 INTP인 경우
SELECT *
FROM USER_INFO
...
--EMPLOYEE에서 부서코드가(DEPT_CODE)가 D5,D6,D8인 사원들 조회
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5','D6','D8');
--직급코드(JOB_CODE)가 J7이거나J2인 사원들 중 급여가200만원인 사원들 조회
--IN 연산자 사용/OR 연산자 사용
SELECT *
FROM EMPLOYEE
...
--EMPLOYEE에서 전체 사원명,보너스 조회(보너스 기준 오름차순 정렬)
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
--ORDER BY BONUS NULLS FIRST; --오름차순인 경우에는 NULL이 맨뒤에
...
*/
--EMPLOYEE에서 부서별 평균 300만원 이상인 직원의
--평균 급여(SALARY)를 조회(부서코두-DEPT_CODE)
...
--직급별(JOB_CODE) 총 급여의 합이1000만원 이상인 직급만 조회
