...
1. 단일행 서브쿼리
-서브쿼리의 조회 결과값이 개수가 오로지1개일 때 (한 행 한 열)
-일반 비교연산자 사용 가능 : =, != , >, <, >= ,<=
*/
...
IN서브쿼리 : 여러 개의 결과값 중에서 한 개라도 일치하는 값이 있다면
*/
...
SELECT *
FROM EMPLOYEE E
WHERE SALARY = (SELECT MAX(SALARY) FROM EMPLOYEE WHERE DEPT_CODE = E. DEPT_CODE)
OR SALARY = (SELECT MAX(SALARY) FROM EMPLOYEE WHERE DEPT_CODE IS NULL);

--사원에 대해 사번, 이름,부서 코드, 구분(사수/사원) 조회
--MANAGER_ID가 있다면 사수가 있다는 소리 즉, 사원 없다면 사수가 없으니 본인이 사수
/*
...
*/
--대리 직급이에도 불구하고 과장 직급들의 최소 급여보다 많이 받는 직원의 사번, 이름, 직급명, 조회
SELECT MIN(SALARY)
FROM EMPLOYEE
...
*/
--전 직원 중 급여가 가장 높은 상위5명 순위,이름,급여 조회
...
FROM(SELECT DEPT_CODE, ROUND(AVG(NVL(SALARY,0))) AS AVG_SALARY
FROM EMPLOYEE
GROUP BY DEPT_CODE 
...
-DENSE_RANK() OVER(정렬 기준): 동일한 순위 이후 등수를 무조건 1씩 증가
*/
--사원별 급여가 높은 순서대로 조회
