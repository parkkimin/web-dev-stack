...
-ROLLUP(컬럼1,컬럼2): 컬럼2을 가지고 다시 중간집계를 내는 함수
...
SELECT JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY CUBE(JOB_CODE); -- 컬럼이 하나일 때는 ROLLUP과 차이X
...
0을 반환, 아니면1을 반환하는 함수
*/
SELECT JOB_CODE, DEPT_CODE, SUM(SALARY), GROUPING(DEPT_CODE), GROUPING(JOB_CODE)
FROM EMPLOYEE
GROUP BY ROLLUP(DEPT_CODE, JOB_CODE);
...
INSERSECT(교집합): 두 쿼리문을 수행한 결과값에 중복된 결과값만 추출 
...
--3. INTERSECT
--부서코드가D5이면서 급여가300만원 초과인 사원들 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
INTERSECT
