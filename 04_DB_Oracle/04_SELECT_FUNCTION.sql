...
SELECT
LTRIM('     K H    '), --왼쪽 공백 제거
LTRIM('ACVAACCCKH','ABC'),--왼쪽에서 ABC에 해당하는 문자들 제거 
RTRIM('46548518KH54569','0123456789')--오른쪽에서 해당하는 숫자들 제거
FROM DUAL;
...
TRIM(TRAILING'Z'FROM'ZZZKHZZZ'),--RTIRM
TRIM(BOTH'Z'FROM'ZZZKHZZZ'),--양쪽 모두 제거
TRIM('            KH      ')
FROM DUAL;

/*
SUBSTR(컬럼,시작 위치,추출길이)
-문자열에서 특정 부분을 잘라서 추출
*/
SELECT 
...
-해당 월에 마지막 날짜 반환
*/
SELECT LAST_DAY(SYSDATE), LAST_DAY('20250815'), LAST_DAY('2025/12/16')
FROM DUAL;
/*
EXTRACT(YEAR|MONTH|DAY FROM날짜)
...
SELECT NVL(MBTI,'MBTI모름')
FROM USER_INFO;

--COALESCE(값1,값2,....) 여러값 가능 
...
-비교하고자 하는 값이 조건값과 일치하는 겨우 그에 해당하는 결과값 반환
...
--직급 코드가 J7인 사원 급여를 10%인상
--직급 코드가 J6인 사원 급여를 15%인상
--직급 코드가 J5인 사원 급여를 20%인상
--그 외 직급 사원 급여 5%인상
--정렬:직급코드(JOB_CODE)J1부터, 인상된 급여 높은 순서대로 SALARY
SELECT 
JOB_CODE,SALARY,
...
/*
그룹 함수 -> 결과값 1개
-대량의 데이터들로 집계 통계 같은 작업을 처리해야 하는 경우
...
AVG
-해당 컬럼값들의 평균값
-모든 그룹 함수는 NULL 값을 자동으로 제외하기 때문에  NVL 함수량 함께 사용할 것을 권장
*/
-- USER_INFO에서 평균 나이
SELECT
...
-DISTINCT: 해당 컬럼값의 중복을 제거한 행 개수 반환
*/
