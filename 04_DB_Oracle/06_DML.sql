...
INSERT ALL INTO 테이블1(컬럼,..)VALUES(값,...)
INTO 테이블2(컬럼,..)VALUES(값,...)
서브쿼리;

INSERT ALL
WHEN 조건1 THEN INTO 테이블1(컬럼,..)VALUES(값,...)
WHEN 조건2 THEN INTO 테이블2(컬럼,..)VALUES(값,...)
서브쿼리;
*/
...
--EMPLOYEE에서 입사일 기준으로 2000년 1월1일 이전 입사한 사원과
--이후 입사한 사원의 테이블을 분리
...
SET MBTI = 'ENTJ'
WHERE MBTI = 'ISFP';
--EMPLOYEE에서 EMP_NAME이 선동일인 사람의 급여(SAKARY)를, 보너스를0.2로 변경
UPDATE EMPLOYEE
...
--TRUNCATE를 사용해서 사람들 전체 삭제 ->ROLLBACK 적용X
TRUNCATE TABLE USER_02;
ROLLBACK;
SELECT * FROM USER_02;
