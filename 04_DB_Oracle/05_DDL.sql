/*
DDL(Data Definition Language): 데이터 정의DJ
-실제 데이터 값이 아닌 구조 자체를 정의하는 언어
-객체(스키마)를 만들고(CREATE),변경하고(ALTER),삭제(DROP)하는 언어
-DB관리자,설계자가 사용

오라클에서 제공하는 객체(구조)/스키마(Schema):테이블(TABLE),뷰(VIEW),시퀀스(SEQUENCE),인덱스(INDEX),
...
-사용자가 객체를 생성하거나 객체를 변경하는 등의 직업을 할 때
데이터베이스 서버에 의해서 자동으로 갱신되는 테이블

*/
-- USER_TABLES는 사용자가 가지고 있는 테이블들의 전반적인 구조를 확인할 수 있는 시스템 테이블
SELECT *
FROM USER_TABLES;
...
COMMENT ON COLUMN 테이블명.컬럼명IS'주석내용';
*/
COMMENT ON COLUMN MEMBER.MEM_ID IS '회원번호';

--테이블에 데이터 추가시키는 구문(DML:INSERT)
--INSERT INTO 테이블명 VALUES(값,값,값,..);
INSERT INTO MEMBER VALUES(1,'user01','pass01','윤의진','남','010-1111-2222','aaa@naver.com','25/06/06'); 
...
/*
제약조건(CONSTRAINTS)
-사용자가 원하는 조건에 데이터만 유지하기 위해서 각 컬럼에 대해 저장될 값에 대한 제약조건을 설정
...
/*
NOT NULL 제약조건
-해당 컬럼에 반드시 값이 존재해야만 하는 경우(즉, 해당 컬럼에 절대NULL이 들어와서는 안되는 경우)
...
INSERT INTO NOTNULL VALUES(1,'user01','pass01','윤의진','남','010-1111-2222','aaa@naver.com','25/06/06'); 
INSERT INTO NOTNULL(MEM_NO,MEM_ID,MEM_PWD,MEM_NAME) VALUES(2,'user02','pass02','이진용'SYSDATE);
...
INSERT INTO MEM_UNIQUE VALUES(1,'user01','pass01','윤의진','남','010-1111-2222','aaa@naver.com','25/06/06'); 
INSERT INTO MEM_UNIQUE(MEM_NO,MEM_ID,MEM_PWD,MEM_NAME) VALUES(2,'user02','pass02','이진용'SYSDATE);
INSERT INTO MEM_UNIQUE(MEM_NO,MEM_ID,MEM_PWD,MEM_NAME,GENDER,MEM_DATE)VALUES(3,'USER03','PASS03','곽병현','0',SYSDATE);
/*
CHECK(조건식) 제약조건
...
INSERT INTO MEM_CHECK VALUES(1,'user01','pass01','윤의진','남','010-1111-2222','aaa@naver.com','25/06/06'); 
...
INSERT INTO MEM_PRI VALUES(1,'user01','pass01','윤의진','남','010-1111-2222','aaa@naver.com','25/06/06'); 
...
INSERT INTO MEM_SEQ VALUES(MEM_SEQ.NEXTVAL,'user01','pass01','윤의진','남','010-1111-2222','aaa@naver.com','25/06/06'); 
INSERT INTO MEM_SEQ(MEM_NO,MEM_ID,MEM_PWD,MEM_NAME,MEM_DATE,GENDER) VALUES(MEM_SEQ.NEXTVAL,'user02','pass02','이진용',SYSDATE,'남');
INSERT INTO MEM_SEQ(MEM_NO,MEM_ID,MEM_PWD,MEM_NAME,GENDER,MEM_DATE,EMAIL)VALUES(MEM_SEQ.NEXTVAL,'USER03','PASS03','곽병현','남',SYSDATE,'BBB@google.com');
...
VALUES('user01','pass01','윤의진','남','010-1111-2222','aaa@naver.com','25/06/06'); 
...
*삭제옵션: 부모테이블에 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리할 건지
...
-- 도서관리 프로그램 테이블들
--테이블명:PUBLISHER
--컬럼:PUB_NO -숫자,기본키,PUB_NAME-문자(50) NULL허용X,PHONE-문자(20)
...
--테이블명:BOOK
--컬럼:BK_NO - 숫자 기본키, BK_TITLE-문자(50)NULL X,BK_AUTHOR문자(50) NULL X
--PUB_NO-숫자,외래키(PUBLISHER의PUB_NO,삭제조건 CASCADE)
CREATE TABLE BOOK(
...
INSERT INTO BOOK(BK_TITLE,BK_AUTHOR,PUB_NO)
VALUES('왜 항상 아가리로만 할까?','이창현',1);
SELECT * FROM BOOK;

--테이블명:MEMBER(위에 있으니깐 삭제)
--컬럼: MEMBER_NO -숫자,기본키,MEMBER_ID-문자(50),중복허용X
DROP TABLE MEMBER;
CREATE TABLE MEMBER(
...
SELECT * FROM MEMBER;
--테이블명:RENT
--컬럼:RENT_NO-숫자,기본키,MEMBER_NO-숫자,외래키(MEMBER의MEMBER_NO,삭제 조건 SET NULL)
...
-기본값 변경MODIFY 컬럼명 DEFAULT 변경할 기본값
*/
DESC USER_INFO;
...
2.제약조건
2-1.제약조건 추가
ADD[CONSTRAINT 계약조건명]
-PRIMARY KEY: PRIMARY KEY(컬럼명)
-FOREIGN KEY: FOREIGN KEY(컬럼명) REFERENCES 테이블명(컬럼명)
-UNIQUE; UNIQUE(컬럼명)
-CHECK: CHECK(컬럼메 대한 조건)

NOTNULL: MODIFY 컬럼명[CONSTRAINT 제약조건명]NOT NULL
*/
--제약조건명 없이 추가
