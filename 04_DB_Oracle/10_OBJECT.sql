...
*/
-- EMP_COPY 테이블의 모든 테이터를 삭제하는 프러시저 생성
...
--사번을 입력 받아서 해당하는 사원의 이름,급여를 전한달하는 프로시저 생성
--프로시저명 : SEL_EMP_ID
...
/*
함수(FUNCTION)
-프로시저와 거의 유사한 용도로 사용하지만 실행 결과를 돌려 받을 수 있다.
 
 CREATE OR REPLACE PROCEDURE FUNCTION 함수명(
        매개변수 데이터타입, ...
    )
    RETURN 데이터타입
    IS 선언부
...
PCOD NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
PADTE DATE DEFAULT SYSDATE, --상품입출고일
AMOUNT NUMBER,--입출고수량
...
--TB_PRODETAIL 테이블에 INSERRT 발생시 
--TB_PRODUCT 테이블에 매번 자동으로 재고수량 UPDATE 되게끔 트리거 정의
--트리거명: TRG_PRO
CREATE OR REPLACE TRIGGER TRF_PRO
